...
//CartaPorte
$cartaPorte = $comprobante->searchNode('cfdi:Complemento')->searchNode('cartaporte30:CartaPorte');
...
<page backbottom="10mm">
    ...
    <?php if (null !== $cartaPorte) : ?>
        <div class="panel">
            <div class="title">Carta Porte IdCCP: <?= $this->e($cartaPorte['IdCCP']) ?></div>
            <div class="content">

                <span>Versión: <?= $cartaPorte['Version'] ?></span>
                <span>TranspInternac: <?= $cartaPorte['TranspInternac'] ?></span>
                <span>Total Distancia Recorrida: <?= $cartaPorte['TotalDistRec'] ?></span>
        ...
        <?php
        $ubicaciones = $cartaPorte->searchNodes('cartaporte30:Ubicaciones', 'cartaporte30:Ubicacion');
        $ubicacionesCount = $ubicaciones->count();
        $ubicacionCounter = 0;
        ?>

        <?php foreach ($ubicaciones as $ubicacion) : ?>
            <?php
            $ubicacionCounter = $ubicacionCounter + 1;
            $domicilioUbicacion = $ubicacion->searchNodes('cartaporte30:Domicilio',);
            ?>
            <div class="panel">
                <div class="title">Ubicación: <?= $this->e($ubicacionCounter) ?> de <?= $this->e($ubicacionesCount) ?></div>
                <div class="content">
                    <p><strong>Tipo Ubicación: </strong><?= $this->e($ubicacion['TipoUbicacion']) ?></p>
                    <p>
                        <span>IDUbicacion: <?= $this->e($ubicacion['IDUbicacion'] ?: '(ninguno)') ?>,</span>
                        <span>RFC RemitenteDestinatario: <?= $this->e($ubicacion['RFCRemitenteDestinatario']) ?>,</span>
                        <span>Nombre Remitente Destinatario: <?= $this->e($ubicacion['NombreRemitenteDestinatario']) ?>,</span>
                        <span>Fecha/Hora Salida/Llegada: <?= $this->e($ubicacion['FechaHoraSalidaLlegada']) ?>,</span>


                        <?php foreach ($domicilioUbicacion as $domilicio) : ?>

                            <span>Localidad: <?= $this->e($domilicio['Localidad']) ?>,</span>
                            <span>Referencia: <?= $this->e($domilicio['Referencia']) ?>,</span>
                            <span>Municipio: <?= $this->e($domilicio['Municipio']) ?>,</span>
                            <span>Estado: <?= $this->e($domilicio['Estado']) ?>,</span>
                            <span>Pais: <?= $this->e($domilicio['Pais']) ?>,</span>
                            <span>CodigoPostal: <?= $this->e($domilicio['CodigoPostal']) ?></span>
        ...
        <?php
        $mercancias = $cartaPorte->searchNodes('cartaporte30:Mercancias');
        $mercanciaCount = $mercancias->count();
        $mercanciasCounter = 0;
        ?>
        ...
        <?php foreach ($mercancias as $mercancia) : ?>
            <?php
            $mercanciasCounter = $mercanciasCounter + 1;
            ?>
            <div class="panel">
                <div class="title">Mercancia: <?= $this->e($mercanciasCounter) ?> de <?= $this->e($mercanciaCount) ?></div>
                <div class="content">
                    <p><strong>Peso Bruto Total: </strong><?= $this->e($mercancia['PesoBrutoTotal']) ?></p>
                    <p>
                        <span>UnidadPeso: <?= $this->e($mercancia['UnidadPeso'] ?: '(ninguno)') ?>,</span>
                        <span>Num Total Mercancias: <?= $mercanciaCount ?>,</span>

                        <?php
                        $merCount = 0;
                        ?>

                        <?php foreach ($mercancia as $mer) : ?>
                            ...
                            <?php if ($merCount <= $mercanciaCount): ?>

                                <span>BienesTransp: <?= $this->e($mer['BienesTransp']) ?>,</span>
                                <span>Descripcion: <?= $this->e($mer['Descripcion']) ?>,</span>
                                <span>Cantidad: <?= $this->e($mer['Cantidad']) ?>,</span>
                                <span>Unidad: <?= $this->e($mer['Unidad']) ?>,</span>
                                <span>MaterialPeligroso: <?= $this->e($mer['MaterialPeligroso']) ?>,</span>
                                <span>PesoEnKg: <?= $this->e($mer['PesoEnKg']) ?></span>

                                <?php
                                $mercanciaTransporta = $mer->searchNodes('cartaporte30:CantidadTransporta');
                                ?>

                                <?php foreach ($mercanciaTransporta as $mt) : ?>
                                    <span>Cantidad Transporta: <?= $this->e($mt['Cantidad']) ?>, </span>
                                    <span>IDOrigen: <?= $this->e($mt['IDOrigen']) ?>, </span>
                                    <span>IDDestino: <?= $this->e($mt['IDDestino']) ?></span>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        <?php endforeach; ?>
        ...
        <?php
        $autotransporte = $cartaPorte->searchNodes('cartaporte30:Mercancias', 'cartaporte30:Autotransporte');
        $identificacionVehicular = $cartaPorte->searchNodes('cartaporte30:Mercancias', 'cartaporte30:Autotransporte', 'cartaporte30:IdentificacionVehicular');
        $seguro = $cartaPorte->searchNodes('cartaporte30:Mercancias', 'cartaporte30:Autotransporte', 'cartaporte30:Seguros');
        $remolque = $cartaPorte->searchNodes('cartaporte30:Mercancias', 'cartaporte30:Autotransporte', 'cartaporte30:Remolques', 'cartaporte30:Remolque');
        ?>
        ...
        <div class="panel">
            ...
            <div class="content">
                ...
                <?php foreach ($autotransporte as $aut) : ?>
                    <p><strong>PermSCT: </strong><?= $this->e($aut['PermSCT']) ?></p>
                    <p>
                        <span>NumPermisoSCT: <?= $this->e($aut['NumPermisoSCT'] ?: '(ninguno)') ?>,</span>

                    </p>
                <?php endforeach; ?>

                <?php foreach ($identificacionVehicular as $idV) : ?>
                    <p><strong>ConfigVehicular: </strong><?= $this->e($idV['ConfigVehicular']) ?></p>
                    <p>
                        <span>PesoBrutoVehicular: <?= $this->e($idV['PesoBrutoVehicular'] ?: '(ninguno)') ?>,</span>
                        <span>Placa: <?= $this->e($idV['PlacaVM'] ?: '(ninguno)') ?>,</span>
                        <span>Año Modelo: <?= $this->e($idV['AnioModeloVM'] ?: '(ninguno)') ?>,</span>

                    </p>
                <?php endforeach; ?>


                <?php foreach ($seguro as $seg) : ?>
                    <p><strong>Asegura Resp Civil: </strong><?= $this->e($seg['AseguraRespCivil']) ?></p>
                    <p>
                        <span>Poliza Resp Civil: <?= $this->e($seg['PolizaRespCivil'] ?: '(ninguno)') ?>,</span>


                    </p>
                <?php endforeach; ?>

                <?php foreach ($remolque as $rem) : ?>
                    <p><strong>SubTipo Rem: </strong><?= $this->e($rem['SubTipoRem']) ?></p>
                    <p>
                        <span>Placa: <?= $this->e($rem['Placa'] ?: '(ninguno)') ?>,</span>
        ...
        <?php
        $FiguraTransporte = $cartaPorte->searchNodes('cartaporte30:FiguraTransporte');
        $tiposFigura = $cartaPorte->searchNodes('cartaporte30:FiguraTransporte', 'cartaporte30:TiposFigura');
        $domicilioFigura = $cartaPorte->searchNodes('cartaporte30:FiguraTransporte', 'cartaporte30:TiposFigura', 'cartaporte30:Domicilio');
        ?>
    ...
    <?php foreach ($tiposFigura as $fig) : ?>
                    <p><strong>Nombre Figura: </strong><?= $this->e($fig['NombreFigura']) ?></p>
                    <p>
                        <span>Tipo Figura: <?= $this->e($fig['TipoFigura'] ?: '(ninguno)') ?>,</span>
                        <span>RFC Figura: <?= $this->e($fig['RFCFigura'] ?: '(ninguno)') ?>,</span>
                        <span>Num Licencia: <?= $this->e($fig['NumLicencia'] ?: '(ninguno)') ?>,</span>


    <?php endforeach; ?>

                    <?php foreach ($domicilioFigura as $df) : ?>

                        <span>Municipio: <?= $this->e($df['Municipio'] ?: '(ninguno)') ?>,</span>
                        <span>Estado: <?= $this->e($df['Estado'] ?: '(ninguno)') ?>,</span>
                        <span>Pais: <?= $this->e($df['Pais'] ?: '(ninguno)') ?>,</span>
                        <span>CodigoPostal: <?= $this->e($df['CodigoPostal'] ?: '(ninguno)') ?>,</span>
